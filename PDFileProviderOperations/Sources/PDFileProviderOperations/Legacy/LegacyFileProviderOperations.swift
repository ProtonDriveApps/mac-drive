...
/// Pre-SDK implementations of file provider operations.
public final class LegacyFileProviderOperations: FileProviderOperationsProtocol {
    ...
    public func item(for identifier: NSFileProviderItemIdentifier,
        ...
        let progress = itemProvider.itemProgress(
            ...
            guard itemProgress?.isCancelled != true else {
                Log.event(.fetchContents(.failed(.init(
                    id: identifier.logIdentifier, error: CocoaError(.userCancelled)
                ))))
            ...
            if let item {
                Log.event(.fetchItem(.succeeded(.init(
                    itemID: item.itemIdentifier.logIdentifier,
                    parentIDs: parentIDFetcher.fetchParentIDs(for: item.itemIdentifier.logIdentifier),
                    isFolder: item.isFolder
                ))))
            } else {
                Log.event(.fetchItem(.failed(.init(id: identifier.logIdentifier, error: error))))
            }

            let fpError = error.mapToFileProviderErrorIfPossible()
    ...
    public func fetchContents(itemIdentifier: NSFileProviderItemIdentifier,
...
#if os(macOS)
        fatalError("macOS app uses SDK for downloading files")
#else
        ...
        let progress = itemProvider.legacyFetchContents(
            ...
            guard fetchContentsProgress?.isCancelled != true else {
                Log.event(.fetchContents(.failed(.init(
                    id: itemIdentifier.logIdentifier, error: CocoaError(.userCancelled).logDescription
                ))))
            ...
            if let item {
                Log.event(.fetchContents(.succeeded(.init(
                    itemID: item.itemIdentifier.logIdentifier,
                    parentIDs: parentIDFetcher.fetchParentIDs(for: item.itemIdentifier.logIdentifier),
                    fetchedVersion: item.itemVersion?.sha256 ?? "-"
                ))))
            } else {
                Log.event(.fetchContents(.failed(.init(
                    id: itemIdentifier.logIdentifier, error: error?.logDescription ?? "-"
                ))))
            }
    ...
    public func createItem(basedOn itemTemplate: NSFileProviderItem,
        ...
        let progress = itemActionsOutlet.createItem(
            ...
            if let item {
                Log.event(.createItem(.succeeded(.init(
                    itemID: item.itemIdentifier.logIdentifier,
                    parentIDs: parentIDFetcher.fetchParentIDs(for: item.itemIdentifier.logIdentifier),
                    createdVersion: item.itemVersion?.sha256 ?? "-"
                ))))
            } else {
        ...
        didStartFileUploadOperation(with: progress, using: pipeline)
                                                         ...
                                                         _ error: Error?) -> Void) -> Progress
    {
        ...
        let progress = itemActionsOutlet.modifyItem(
            ...
            options: options, request: request, pool: tower.storage.backgroundContextPool
            ...
            guard modifyItemProgress?.isCancelled != true else {
                Log.event(.modifyItem(.failed(.init(
                    id: item.itemIdentifier.logIdentifier,
                    error: CocoaError(.userCancelled)
                ))))
            ...
            if let modifiedItem {
                Log.event(.modifyItem(.succeeded(.init(
                    itemID: modifiedItem.itemIdentifier.logIdentifier,
                    parentIDs: parentIDFetcher.fetchParentIDs(for: modifiedItem.itemIdentifier.logIdentifier),
                    stillPendingFields: .init(fields),
                    version: modifiedItem.itemVersion?.sha256 ?? "-"
                ))))
            } else {
                Log.event(.modifyItem(.failed(.init(id: item.itemIdentifier.logIdentifier, error: error))))
            }
                           ...
                           completionHandler: @escaping (Error?) -> Void) -> Progress
    {
        ...
        // otherwise it might fail and cause another session forking
        guard !tower.sessionCommunicator.isWaitingforNewChildSessionAvailability.value else {
            Log.event(.deleteItem(.failed(.init(id: identifier.logIdentifier, errorMessage: "No child session"))))
        ...
        let progress = itemActionsOutlet.deleteItem(
            tower: tower, identifier: identifier, baseVersion: version, options: options, request: request, pool: tower.storage.backgroundContextPool
        ) { [weak self] error in
            ...
            guard deleteItemProgress?.isCancelled != true else {
                Log.event(.deleteItem(.failed(.init(id: identifier.logIdentifier,
                                                     error: CocoaError(.userCancelled)))))
            ...
            if let error {
                Log.event(.deleteItem(.failed(.init(id: identifier.logIdentifier, error: error))))

            } else {
                Log.event(.deleteItem(.succeeded(.init(
                    itemID: identifier.logIdentifier,
                    parentIDs: parentIDFetcher.fetchParentIDs(for: identifier.logIdentifier)
                ))))
            }
...
private extension LegacyFileProviderOperations {
    func didStartFileUploadOperation(with progress: Progress?,
                                     using pipeline: DriveObservabilityPipeline = .legacy) {
        guard let progress else { return }
        uploadCollector.startObserving(progress: progress, using: pipeline)
    }

    func didFinishFileUploadOperation(with progress: Progress?,
                                      using pipeline: DriveObservabilityPipeline = .legacy) {
        guard let progress else { return }
        uploadCollector.finishObserving(progress: progress, using: pipeline)
    }
